    ...
    <div className="space-y-8">
          ...
          <div className="border border-gray-200 rounded-2xl p-6 bg-white">
            ...
            <form onSubmit={createUser} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[12px] font-medium text-gray-700 font-sans mb-1.5">
                    Naam (optioneel)
                  </label>
                  <input
                    type="text"
              ...
              <div className="max-w-xs">
                <label className="block text-[12px] font-medium text-gray-700 font-sans mb-1.5">
                  Tijdelijk wachtwoord <span className="text-[#F75D20]">*</span>
                </label>
                <input
                  type="text"
                  value={newPassword}
          ...
          <div>
            ...
            {!usersLoading && users.length === 0 && !usersError && (
              <p className="text-gray-400 font-sans text-sm">Nog geen gebruikersaccounts aangemaakt.</p>
            )}
            <div className="space-y-2">
              {users.map((u) => (
                <div key={u.uid} className="flex items-center justify-between gap-4 px-5 py-3.5 border border-gray-200 rounded-xl bg-white">
                  <div>
                    {u.displayName && (
                      <p className="text-sm font-medium text-gray-900 font-sans">{u.displayName}</p>
                    )}
